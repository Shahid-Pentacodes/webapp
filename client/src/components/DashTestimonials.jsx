...
export default function DashTestimonial() {
  ...
  useEffect(() => {
    const fetchTestimonial = async () => {
      try {
        const res = await fetch(`/api/testimonial/gettestimonials?userId=${currentUser._id}`);
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/testimonial/gettestimonials?userId=${currentUser._id}&startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  const handleDeleteService = async () => {
    ...
    try {
      const res = await fetch(
        `/api/testimonial/deletetestimonial/${testimonialIdToDelete}/${currentUser._id}`,
        {
          method: 'DELETE',
        }
      );
      ...
      if (!res.ok) {
        ...
        setUserTestimonial((prev) =>
          prev.filter((testimonial) => testimonial._id !== testimonialIdToDelete)
        );
      }
    } catch (error) {
      console.log(error.message);
