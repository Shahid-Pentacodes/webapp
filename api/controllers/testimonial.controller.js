...
export const create = async (req, res, next) => {
  if (!req.user.isAdmin) {
    return next(errorHandler(403, 'You are not allowed to create a testimonial'));
  }
  if (!req.body.title || !req.body.count) {
...
export const deletetestimonial = async (req, res, next) => {
  if (!req.user.isAdmin || req.user.id !== req.params.userId) {
    return next(errorHandler(403, 'You are not allowed to delete this testimonial'));
  }
  try {
...
export const updatetestimonial = async (req, res, next) => {
  if (!req.user.isAdmin || req.user.id !== req.params.userId) {
    return next(errorHandler(403, 'You are not allowed to update this testimonial'));
  }
  try {
